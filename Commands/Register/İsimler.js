...
module.exports = {
    ...
    async execute(client, message, args, embed) {
        ...
        if (!member) return message.reply({ embeds: [embed.setDescription(`${emojis.Register.cancelEmoji}Lütfen bir ${!member ? "üye" : ""} belirtiniz.`)] }).delete(5)

        const data = await userSchema.findOne({ guildID: message.guild.id, userID: member.id })
        if (!data || !data.names.length > 0) return message.reply({ embeds: [embed.setDescription(`${emojis.Register.cancelEmoji} ${member} kullanıcısının isim geçmişi bulunamadı.`)] }).delete(5)

        const names = data.names.reverse().slice(0, 15).map((x, i) => `\` ${i+1} \` \` ${x.name} \` (${x.type} - ${x.executor})`).join("\n")
        message.reply({ embeds: [embed.setDescription(`${emojis.Register.warningEmoji} ${member} kullanıcısının **${data.names.length}** adet geçmiş ismi bulundu;\n\n${names}`)] }).delete(15)
