...
module.exports = {
    ...
    async execute(client, message, args, embed) {
        ...
        if(member.roles.highest.position >= message.member.roles.highest.position) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Belirtilen üye kayıtsıza gönderemezsin.`)] }).delete(5)
        if(roles.Register.unregRoles.some(x => member.roles.cache.has(x))) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Belirtilen üye zaten kayıtsızda.`)]})
        ...
        message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.verifyEmoji} ${member} kullanıcısı başarıyla kayıtsıza gönderildi.`)] }).delete(10)
        client.channels.resolve(channels.Logs.unRegLog).send({ content: `${botEmojis.Register.okEmoji} ${member}, **${message.author}** tarafından <t:${Math.floor(Date.now() / 1000)}> tarihinde (<t:${Math.floor(Date.now() / 1000)}:R>) kayıtsıza atıldı.` })
