...
module.exports = {
    ...
    async execute(client, message, args, embed) {
        ...
        if(member.id === message.author.id) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} endini sustaramazsın.`)] }).delete(5)
        ...
        async function vmute (user, reason, time, timeLength, button, count) {
            ...
            await button.channel.send({ embeds: [
                embed.setDescription(`${message.member}, ${user} adlı kullanıcı başarıyla __ses__ kanallarında susturuldu!`)
            ] })
            ...
            vmute.wsend({
                embeds: [
                    ...
                    .addFields({
                        ...
                        ${botEmojis.Register.okEmoji} İşlem: Ses Kanallarında Susturma (\`Voice Mute\`)
