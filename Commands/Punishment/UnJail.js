...
module.exports = {
    name: 'unjail',
    aliases: ['unceza', 'unhapis', 'uncezalı', 'unjailed', 'unja', 'unkodes'],
    ...
    async execute(client, message, args, embed) {
        ...
        if(member.id === message.author.id) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Kendini hapisten çıkaramazsın.`)] }).delete(5)
        if(member.id === client.user.id) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Botu hapisten çıkaramazsın.`)] }).delete(5)
        if(member.id === message.guild.ownerId) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Sunucu sahibini hapisten çıkaramazsın.`)] }).delete(5)
        if(member.roles.highest.position >= message.member.roles.highest.position) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Belirtilen üyeyi hapisten çıkaramazsın.`)] }).delete(5)
        if(!member.roles.cache.has(botRoles.Silent.Suspended)) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Belirtilen üye zaten hapisten çıkarılmış.`)] }).delete(5)
        
        let data = await punishSchema.findOne({ userID: member.id, punishmentContinue: true, punishmentType: "JAIL" })
        if(!data) return message.reply({ embeds: [embed.setDescription(`Belirtilen kullanıcının __ses__ ve __metin__ kanallarından uzaklaştırma cezası bulunmuyor.`)] }).delete(5)
        ...
        ${botEmojis.Register.okEmoji} İşlem: Ses ve Metin kanallarına erişim açıldı (\`UN-JAIL\`)
        ...
        message.reply({ embeds: [embed.setDescription(`${member} (\`${member.id}\`) adlı kullanıcının __ses__ ve __metin__ kanallarına erişimi açıldı.`)] }).delete(10)
