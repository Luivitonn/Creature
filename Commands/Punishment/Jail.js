...
module.exports = {
    name: 'jail',
    aliases: ['ceza', 'hapis', 'cezalı', 'jailed', 'jail', 'kodes'],
    ...
    async execute(client, message, args, embed) {
        ...
        if(member.id === message.author.id) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Kendini hapise atamazsın.`)] }).delete(5)
        if(member.id === client.user.id) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Botu hapise atamazsın.`)] }).delete(5)
        if(member.id === message.guild.ownerId) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Sunucu sahibini hapise atamazsın.`)] }).delete(5)
        if(member.roles.highest.position >= message.member.roles.highest.position) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Belirtilen üyeyi hapise atamazsın.`)] }).delete(5)
        if(member.roles.cache.has(botRoles.Silent.Suspended)) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Belirtilen üye zaten hapise atılmıs.`)] }).delete(5)
        let saveRoles = await member.roles.cache.filter(x => x.id !== member.guild.id && x.id !== botRoles.Register.boosterRoles).map(x => x.id)
        ...
        async function vmute (user, reason, time, timeLength, button, count) {
            ...
            vmute.wsend({
                embeds: [
                    ...
                    .setTimestamp()
                    .setDescription(`${user} (\`${user.id}\`) adlı kullanıcı ${button.user} tarafından __ses__ ve __metin__ kanallarında uzaklaştırıldı.`)
                    .addFields({
                        ...
                        ${botEmojis.Register.okEmoji} İşlem: Ses ve Metin Kanallarından Uzaklaştırılma (\`Jail\`)
