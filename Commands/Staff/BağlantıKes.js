...
module.exports = {
    ...
    async execute(client, message, args, embed) {
        ...
        if(!member) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Lütfen bir ${member == undefined ? "üye" : ""} belirtin.`)] }).delete(10)
        if(!member.voice.channel) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Belirttiğiniz üye ses kanalında bulunmuyor.`)] }).delete(10)

        if(member.voice.channel) {
            member.voice.disconnect([`${botEmojis.Register.verifyEmoji} Bağlantı kesme komutu kullanıldı. Yetkili: ${message.author.tag} (${message.author.id})`]);
            message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.verifyEmoji} ${member} üyesinin ses kanalına erişimi kesildi.`)] })
        }
