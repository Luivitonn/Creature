...
module.exports = {
    ...
    async execute(client, message, args, embed) {
        ...
        if(!idb) return message.reply({ embeds: [embed.setDescription(`${member.id == message.author.id ? `Selam ${member} Veritabanımda ihlal geçmişin bulunmuyor.` : `${member} üyesine ait veritabanımda bir ihlal geçmişi bulunmuyor.`}`)] }).delete(10)

        let total = idb.Ban+idb.Jail+idb.CMute+idb.VMute

        if(member.id === message.author.id) {
            message.reply({ embeds: [embed.setDescription(`Selam ${member}, Toplamda \`${total}\` ihlalin bulunuyor.
            ${codeBlock("fix", `Daha önce verilen cezaların:
            (Ban: ${idb.Ban}, CMute: ${idb.CMute}, VMute: ${idb.VMute}, Jail: ${idb.Jail})`)}`)] }).delete(15)
        } else {
            message.reply({ embeds: [embed.setDescription(`${member} üyesine ait toplamda \`${total}\` ihlali bulunuyor.
