...
module.exports = {
    ...
    permissions: [botRoles.Register.boosterRoles],
    channels: [],
    ownerOnly: false,
    async execute(client, message, args, embed) {
        ...
        let name = args.join(" ")
        if(name.length > 32) return message.channel.send({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} İsim 32 karakterden fazla olamaz.`)]}).delete(5)
        ...
        await member.setNickname(fixName).catch(err => { return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} İsim değiştirme işlemi sırasında bir hata oluştu.`)]}).delete(5) });
        await message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.verifyEmoji} Başarıyla isminizi \`${fixName}\` olarak değiştirdim.`)]})
        client.channels.resolve(botChannels.Logs.nameLog).send({ content: `${member} adını, <t:${Math.floor(Date.now() / 1000)}> tarihinde (<t:${Math.floor(Date.now() / 1000)}:R>) \`${fixName}\` olarak değiştirdi.` })
