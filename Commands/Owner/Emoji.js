...
module.exports = {
    ...
    async execute(client, message, args, embed) {
      ...
      const emoji = args[0];
      if(!emoji) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Bir emoji belirtmelisin.`)]}).delete(5)
      let emojiName = args.slice(1).join("_")
      if(!emojiName) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Bir emoji adı belirtmelisin.`)]}).delete(5)
      ...
      return await message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.verifyEmoji} Emoji başarıyla eklendi. ${directlyEmoji}`)]}).delete(10)

      }

      const parseCustomEmoji = parseEmoji(emoji);
      if(!parseCustomEmoji.id) return message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.cancelEmoji} Lütfen geçerli bir emoji belirtmelisin.`)]}).delete(5)
      ...
      message.reply({ embeds: [embed.setDescription(`${botEmojis.Register.verifyEmoji} Emoji başarıyla oluşturuldu. ${createEmoji}`)]}).delete(10)
